...
from resnet_timesfm import ImprovedHydroTransformer  # <-- model file provided above
...
RUN_TAG        = "pretrained_resnet50_timesfm_09092025"
...
# ALL_WATERSHEDS = [4102700, 4104945, 4105000, 4105500]
# Flow based distribution
VAL_WATERSHEDS = [4113000,4144500,4097500,4146000,4104945,4118500,4166500,4102700,4108600,4154000,4097540,4126970]


# area basd distribution
# VAL_WATERSHEDS = [4117500,4163400,4164100,4146000,4146063,4109000,4112000,4112500,4116000,4118500,4104945,4096405]
...
def build_model(in_channels: int, device: torch.device,
    ...
    # Instantiate the new model (ResNet-50 spatial + deeper Transformer temporal)
    model = ImprovedHydroTransformer(
        in_channels=in_channels,                     # dynamic image channels per timestep
        spatial_d_model=256,               # ResNet→feature width
        spatial_pretrained=True,           # use imagenet weights
        spatial_freeze_stages=1,           # freeze stem (optional)
        # --- PatchTST temporal encoder (ETTh1-pretrained) ---
        temporal_repo="ibm-research/patchtst-etth1-pretrain",
        temporal_out_dim=512,              # output width from temporal encoder
        temporal_pool="mean",              # "mean" or "last"
        # statics + fusion + head
        static_d_model=64,
        fusion_type="film",
        output_dim=1,
    ...
    # Optionally freeze most temporal layers then unfreeze the last N for fine-tuning
    ...
    # Freeze BN stats in the spatial backbone for small batches (stability)
...
def eval_nse_and_plots(model, loader, device, epoch, plots_dir, gmm_stream, flow_transform_kind, use_amp=True):
    ...
    for ws, vals in all_nse.items():
        ...
        plt.savefig(os.path.join(plots_dir, f"watershed_{ws}.png"), dpi=150)
        plt.close()

# =========================
# Data loaders (simple split)
# =========================
...
def main():
    ...
    model = build_model(
        ...
        fusion_type="film",
        freeze_all_temporal=True, unfreeze_last_n=1
    )
    ...
    try:
        for epoch in range(start_epoch + 1, EPOCHS_RUN + 1):
            ...
            # Test NSE + plots each epoch (optional)
            eval_nse_and_plots(
