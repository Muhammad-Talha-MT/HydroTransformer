...
ACCUM_STEPS = 4  # set >1 to enable gradient accumulation
...
def make_loaders():
    train_ds = _make_dataset(ALL_WATERSHEDS, TRAIN_START_YEAR, TRAIN_END_YEAR)
    val_ds   = _make_dataset(ALL_WATERSHEDS, VAL_START_YEAR,   VAL_END_YEAR)
...
def save_checkpoint(path: str, epoch: int, model: nn.Module, opt, best_val: float,
    ...
    state = {
        ...
        "scheduler_state": (scheduler.state_dict() if scheduler is not None else None),  # <-- sched
        "rng_state": {
...
def main():
    ...
    scheduler = CosineAnnealingLR(opt, T_max=EPOCHS, eta_min=5e-6)
    ...
    if args.resume is not None and os.path.isfile(args.resume):
        try:
            start_epoch = load_checkpoint(
                ...
                strict=True,
                load_optim=False,
                scheduler=None,  # <-- resume scheduler
            )
